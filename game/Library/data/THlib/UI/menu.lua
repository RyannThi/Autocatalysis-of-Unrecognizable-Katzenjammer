...
function replay_saver:init(stages, finish, isc_item, exitCallback)
    self.layer = LAYER_TOP + 400
    self.group = GROUP_GHOST
    self.bound = false
    self.x = screen.width * 0.5 - screen.width
    self.y = screen.height * 0.5
    ...
    elseif self.state == 1 then
        ...
        elseif KeyIsPressed("shoot") then
            if self.state2CursorX == 12 and self.state2CursorY == 6 then
                ...
                -- 保存录像
                --lstg.MsgBoxWarn(tostring(self.stages))
                --lstg.MsgBoxWarn(tostring(self.state1Selected))
                --lstg.MsgBoxWarn(tostring(self.state2UserName))
                --lstg.MsgBoxWarn(tostring(self.finish))
                ext.replay.SaveReplay(self.stages, self.state1Selected, self.state2UserName, self.isc_item, self.finish)
